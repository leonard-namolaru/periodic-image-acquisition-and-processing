...
rtems_task Init(rtems_task_argument argument) {
  ...
  assert(status == RTEMS_SUCCESSFUL);
  status = rtems_task_create(processing_task_name, PROCESSING_TASK_PRIORITY, TASK_STACK_SIZE, RTEMS_PREEMPT | RTEMS_NO_TIMESLICE  | RTEMS_ASR | RTEMS_INTERRUPT_LEVEL(0), RTEMS_LOCAL | RTEMS_FLOATING_POINT, &processing_task_id);
  assert(status == RTEMS_SUCCESSFUL);
