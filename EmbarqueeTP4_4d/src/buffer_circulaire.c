...
void init_buffer_circulaire(buffer_circulaire* b, flux* buffer,
...
int push(buffer_circulaire* fifo, flux* source, uint16_t taille) {
	int res = -3;
	if (fifo != 0 && fifo->buffer != 0) {
		if (taille > fifo->taille_donnee) {
			...
			if (i_prochaine_donnee == fifo->i_ancienne_donee) {
				...
				flux* dest = &(fifo->buffer[i_prochaine_donnee
						* fifo->taille_donnee]);
				uint16_t i;
				// copie de la donnée
				for (i = 0; i < taille; i++) {
...
int pop(buffer_circulaire* fifo, flux* destination, uint16_t taille_max) {
	int res = -3;
	if (fifo != 0 && fifo->buffer != 0) {
		if (taille_max < fifo->taille_donnee) {
			...
			if (fifo->i_ancienne_donee == fifo->nombre_donnees) {
				...
				flux* source = &(fifo->buffer[fifo->i_ancienne_donee
						* fifo->taille_donnee]);
				// copie de la donnee
				uint16_t i;
...
unsigned int test_unitaire_buffer_circulaire() {
	...
	init_buffer_circulaire(&bc, (flux*) &buffer_fifo, TAILLE, NB_DONNEES);
	...
	if (res == 0) {
		...
		if (-1 != tmp || buffer_fifo[0][0].id_window != 0 || bc.occupation != 0
				|| bc.occupation_max != 0) {
			res = i;
		}
	}
	...
	if (res == 0) {
		...
		if (0 != tmp || buffer_fifo[0][0].id_window != 0x11 || buffer_fifo[0][1].id_window != 0x22
	...
	if (res == 0) {
		...
		if (-1 != pop(&bc, buffer, TAILLE - 1) || buffer[0].id_window != 0
				|| buffer[0].id_window != 0 || bc.occupation != 1
				|| bc.occupation_max != 1) {
			res = i;
		}
	}
	...
	if (res == 0) {
		...
		if (2 != tmp || buffer[0].id_window != 0x11 || buffer[1].id_window != 0x22
	...
	if (res == 0) {
		...
		buffer[2].id_window = 0;
		tmp = pop(&bc, buffer, TAILLE);
		if (-2 != tmp || buffer[0].id_window != 0 || buffer[1].id_window != 0 || bc.occupation != 0
				|| bc.occupation_max != 1) {
			res = i;
		}
	}
	...
	if (res == 0) {
		...
		res = 0;
		if (0 != tmp || buffer_fifo[1][0].id_window != 0x11 || buffer_fifo[1][1].id_window != 0x12
				|| bc.occupation != 1 || bc.occupation_max != 1) {
			res = i;
		}
	}
	if (res == 0) {
		...
		if (0 != push(&bc, buffer, TAILLE) || buffer_fifo[2][0].id_window != 0x13
				|| buffer_fifo[2][1].id_window != 0x14|| bc.occupation != 2 || bc.occupation_max != 2) {
			res = i;
		}
	}
	if (res == 0) {
		...
		if (0 != push(&bc, buffer, TAILLE) || buffer_fifo[0][0].id_window != 0x17
				|| buffer_fifo[0][1].id_window != 0x18|| bc.occupation != 3 || bc.occupation_max != 3) {
			res = i;
		}
	}
