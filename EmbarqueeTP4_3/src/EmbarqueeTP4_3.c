...
rtems_id acquisition_task_id, processing_task_id, timer_id, message_queue_synchronize_timer_acquisition_id, message_queue_synchronize_acquisition_processing_id;
rtems_name acquisition_task_name, processing_task_name, timer_name, message_queue_synchronize_timer_acquisition_name, message_queue_synchronize_acquisition_processing_name;
...
rtems_task Init(rtems_task_argument argument) {
  ...
  assert(status == RTEMS_SUCCESSFUL);
  status = rtems_task_create(processing_task_name, ACQUISITION_TASK_PRIORITY, TASK_STACK_SIZE, RTEMS_PREEMPT | RTEMS_NO_TIMESLICE  | RTEMS_ASR | RTEMS_INTERRUPT_LEVEL(0), RTEMS_LOCAL | RTEMS_FLOATING_POINT, &processing_task_id);
  assert(status == RTEMS_SUCCESSFUL);
