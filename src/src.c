		...
		if(status == RTEMS_SUCCESSFUL) {
			 ...
			 for (i = 0; i < NB_WINDOWS_TO_PRODUCE_FOR_EACH_CALL_TO_PRODUCE_IMAGES ; i++) {
				...
				assert(status == RTEMS_SUCCESSFUL);
				 ...
				 flux_pondere[i].measures[(flux_pondere[i].id_first_acquisition + acquisition_counter) % FLUX_LENGTH] = computeFluxPondere(window, mask);

				 status = rtems_semaphore_release(semaphore_id);
				 assert(status == RTEMS_SUCCESSFUL);

				 if(flux_pondere[i].measures[FLUX_LENGTH -1] != 0) {
					push_partagee(&circular_buffer, (uint8_t*) &flux_pondere[i], sizeof(flux));
				 }
			 }
		}
	}
...
rtems_task Init(rtems_task_argument argument) {
  ...
  init_buffer_circulaire_partagee(&circular_buffer, (uint8_t*) flux_pondere_fifo, sizeof(flux), NB_WINDOWS_TO_PRODUCE_FOR_EACH_CALL_TO_PRODUCE_IMAGES);
